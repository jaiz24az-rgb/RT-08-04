...
async function startServer() {
  ...
  // API error handling middleware to guarantee JSON responses (e.g. for 413 Payload Too Large)
    ...
    if (err) {
      ...
      res.status(status).json({
        ...
        error: err.type === 'entity.too.large' 
          ? 'Ukuran data terlalu besar untuk dikirim sekaligus ke server lokal' 
          : (err.message || 'Gagal memproses data di server lokal')
      });
      return;
    }
    next();
