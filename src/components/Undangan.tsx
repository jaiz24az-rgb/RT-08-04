...
export default function Undangan({
  ...
  // Migration / normalization effect for existing stored letters
  useEffect(() => {
    if (!lettersList || lettersList.length === 0 || !onUpdateLettersList) return;
    let changed = false;
    const updated = lettersList.map(item => {
      if (/RT-8\/POPOH/i.test(item.nomorSurat) || /RT-08\/POPOH/i.test(item.nomorSurat) || /RT-8\//i.test(item.nomorSurat)) {
        changed = true;
        const newNo = item.nomorSurat
          .replace(/RT-8\/POPOH/gi, 'RT-08,04/TAS-3')
          .replace(/RT-08\/POPOH/gi, 'RT-08,04/TAS-3')
          .replace(/RT-8\//gi, 'RT-08,04/');
        return { ...item, nomorSurat: newNo };
      }
      return item;
    });
    if (changed) {
      onUpdateLettersList(updated);
    }
  }, [lettersList, onUpdateLettersList]);
  ...
  useEffect(() => {
    ...
    const combinedStr = `${rtTitle} ${rtAddress}`;
    const rtMatch = combinedStr.match(/(?:RUKUN TETANGGA|RT)\s*0*(\d+)/i);
    const rwMatch = combinedStr.match(/(?:RUKUN WARGA|RW)\s*0*(\d+)/i);

    const rtVal = rtMatch ? String(parseInt(rtMatch[1], 10)).padStart(2, '0') : '08';
    const rwVal = rwMatch ? String(parseInt(rwMatch[1], 10)).padStart(2, '0') : '04';

    const rtCode = `RT-${rtVal},${rwVal}`;

    let areaCode = 'TAS-3';
    if (/perumtas\s*3|tas\s*3/i.test(combinedStr)) {
      areaCode = 'TAS-3';
    } else {
      const villageName = getDesaName(rtAddress);
      areaCode = villageName || 'TAS-3';
    }
    ...
    setNewLetterNo(`${paddedIdx}/${rtCode}/${areaCode}/${roman}/${year}`);
  ...
  const [letterNumber, setLetterNumber] = useState('001/RT-08,04/TAS-3/VII/2026');
              ...
              <form onSubmit={handleRegisterLetter} className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <input 
                    ...
                    placeholder="Contoh: 001/RT-08,04/TAS-3/VII/2026"
                    required
                  />
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-6 bg-white border border-slate-200 rounded-2xl p-6 shadow-xs space-y-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {/* Hari / Tanggal */}
              <div className="space-y-1.5">
                <label className="block text-xs font-semibold text-slate-700 font-mono flex items-center justify-between">
                  <span className="flex items-center gap-1.5">
                    <Calendar className="w-3.5 h-3.5 text-sky-600" />
                    <span>Hari / Tanggal Kegiatan</span>
                  </span>
                  <span className="text-[10px] text-sky-600 font-bold">Pilih Kalender System 📅</span>
                </label>
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="bg-sky-50 hover:bg-sky-100 border border-sky-200 text-sky-900 text-xs font-bold rounded-xl px-2.5 py-2 cursor-pointer focus:outline-none focus:ring-2 focus:ring-sky-500 transition shrink-0"
                    title="Klik di sini untuk memilih tanggal dari kalender otomatis"
                  />
                  <input
                    type="text"
                    value={dayDate}
                    onChange={(e) => setDayDate(e.target.value)}
                    className="flex-1 w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2 text-xs text-slate-900 focus:outline-none focus:ring-2 focus:ring-sky-500 font-medium font-sans"
                    placeholder="cth: Senin, 27 Juli 2026"
                  />
                </div>
              </div>
                      ...
                      onChange={async (e) => {
                        ...
                        if (file) {
                          try {
                            ...
                            triggerCropper(compressed, 'free', 'Potong Foto Rombong', (cropped) => {
                              setNewRombong(prev => ({
                                ...prev,
                                fotoBase64: cropped,
                    ...
                    <input 
                      ...
                      onChange={async (e) => {
                        ...
                        if (file) {
                          try {
                            const base64 = await compressImage(file);
                            triggerCropper(base64, '1:1', 'Potong Foto Profil', (cropped) => {
                              setNewWarga(prev => ({
                                ...prev,
                                fotoBase64: cropped,
                                fotoNamaFile: file.name
                            ...
                            onChange={async (e) => {
                              ...
                              if (file) {
                                try {
                                  ...
                                  triggerCropper(base64, '1:1', 'Potong Logo Instansi', (cropped) => {
                                    setLocalAppLogo(cropped);
                                  });
                                } catch (err) {
