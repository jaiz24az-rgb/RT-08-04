        ...
        <div className="bg-white border border-slate-250 rounded-2xl p-6 shadow-lg animate-in fade-in slide-in-from-top-4 duration-300 text-slate-800">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-6 bg-slate-50 p-1.5 rounded-2xl border border-slate-150">
            ...
            <button
              ...
              onClick={() => {
                setActiveTab('petty');
                setNewTx({ ...newTx, kategori: 'Petty Kas', tipe: 'pengeluaran', sumberKas: 'rtPettyCash' });
              }}
              className={`flex flex-col sm:flex-row items-center justify-center gap-1.5 py-2 px-1.5 sm:py-2.5 sm:px-3 rounded-xl text-[10px] sm:text-xs font-extrabold transition cursor-pointer text-center sm:text-left ${
                activeTab === 'petty'
            ...
            <button
              ...
              onClick={() => {
                setActiveTab('umum');
                setNewTx({ ...newTx, kategori: 'Kas Umum RT', tipe: 'pengeluaran', sumberKas: 'rtBank' });
              }}
              className={`flex flex-col sm:flex-row items-center justify-center gap-1.5 py-2 px-1.5 sm:py-2.5 sm:px-3 rounded-xl text-[10px] sm:text-xs font-extrabold transition cursor-pointer text-center sm:text-left ${
                activeTab === 'umum'
            ...
            {activeTab === 'petty' && (
              <div className="space-y-4 animate-in fade-in duration-150">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-650 mb-1.5 font-mono">Kategori Transaksi</label>
                    <select
                      required
                    ...
                    >
                      <option value="Petty Kas">1. Petty Kas</option>
                      <option value="Kas Umum RT">2. Kas Umum RT</option>
                      <option value="Kas Rombong">3. Kas Rombong</option>
                    </select>
                  </div>
                          ...
                          onChange={async (e) => {
                            ...
                            if (file) {
                              try {
                                ...
                                triggerCropper(compressed, 'free', 'Potong Bukti Transaksi', (cropped) => {
                                  setNewTx(prev => ({
                                    ...prev,
                                    fotoBase64: cropped,
            ...
            {activeTab === 'bank' && (
              <div className="space-y-4 animate-in fade-in duration-150">
                <div className="bg-emerald-50 text-emerald-900 border border-emerald-200 p-3.5 rounded-xl text-xs leading-relaxed">
                  🏦 <strong>Pemindahbukuan Bank ⇄ Kas Tunai / Operasional:</strong> Berfungsi mencatatkan pencairan dana dari rekening bank ke laci kas operasional (Rombong Tunai, Kas Kecil, atau Kas Umum RT), maupun penyetoran tunai ke rekening bank secara formal.
                </div>
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-3.5 space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2 max-w-md">
                    <button
                      type="button"
                      onClick={() => {
                        setTransferBankSelection('rtBank');
                      }}
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        setTransferBankSelection('rombongBank');
                      }}
                      className={`py-2.5 px-3 rounded-xl text-xs font-bold transition cursor-pointer text-center border ${
                ...
                {/* Target Cash Account Selector Block */}
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-3.5 space-y-2">
                  <div className="text-xs font-semibold text-slate-700 font-mono flex items-center gap-1.5">
                    💵 2. Pilih Pintu Kas Tunai / Operasional (Tujuan / Asal Dana):
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                    <button
                      type="button"
                      onClick={() => setTransferTargetKas('rombongTunai')}
                      className={`py-2.5 px-3 rounded-xl text-xs font-bold transition cursor-pointer text-center border ${
                        transferTargetKas === 'rombongTunai'
                    ...
                    >
                      🍱 Rombong Tunai
                      <div className="text-[10px] font-mono font-medium opacity-80 mt-0.5">
                        Sisa: Rp {activeKas.rombongTunai.toLocaleString('id-ID')}
                      </div>
                    </button>
                    <button
                      type="button"
                      onClick={() => setTransferTargetKas('rtPettyCash')}
                      className={`py-2.5 px-3 rounded-xl text-xs font-bold transition cursor-pointer text-center border ${
                        transferTargetKas === 'rtPettyCash'
                    ...
                    >
                      🪙 Kas Kecil RT
                      <div className="text-[10px] font-mono font-medium opacity-80 mt-0.5">
                        Sisa: Rp {activeKas.rtPettyCash.toLocaleString('id-ID')}
                      </div>
                    </button>
                    <button
                      type="button"
                      onClick={() => setTransferTargetKas('rtTunai')}
                      className={`py-2.5 px-3 rounded-xl text-xs font-bold transition cursor-pointer text-center border ${
                        transferTargetKas === 'rtTunai'
                          ? 'bg-sky-50 text-sky-800 border-sky-300 ring-2 ring-sky-500/10'
                          : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'
                      }`}
                    >
                      💵 Kas Umum / RT Tunai
                      <div className="text-[10px] font-mono font-medium opacity-80 mt-0.5">
                        Sisa: Rp {activeKas.rtTunai.toLocaleString('id-ID')}
                      </div>
                    </button>
                  </div>
                </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder={bankType === 'bank_ke_petty' 
                        ? `Mutasi ${kasLabels[transferBankSelection].label}: Penarikan dana mengisi ${kasLabels[transferTargetKas].label}` 
                        : `Mutasi ${kasLabels[transferBankSelection].label}: Penyetoran sisa ${kasLabels[transferTargetKas].label}`
                      }
                      value={transferDesc}
                      onChange={e => setTransferDesc(e.target.value)}
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500"
