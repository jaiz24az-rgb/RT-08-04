...
export const REASONS_DISKON_PEMBEBASAN = [
  'Pembebasan Lansia / Dhuafa / Yatim',
  'Pengurus / Petugas RT Aktif',
  'Warga Kurang Mampu / Bantuan Sosial RT',
  'Rumah Kosong / Belum Dihuni (Diskon 50%)',
  'Kebijakan Khusus Musyawarah RT',
...
export default function TagihanWarga({ 
  ...
  // Helper to extract year from a slot, whether stored in .tahun or embedded in .bulan string
  const getSlotYear = (slot?: { bulan?: string; tahun?: number | string }, fallbackYear: number = 2026): number => {
    if (!slot) return fallbackYear;
    if (slot.tahun !== undefined && slot.tahun !== null && !isNaN(Number(slot.tahun)) && Number(slot.tahun) > 0) {
      return Number(slot.tahun);
    }
    if (slot.bulan) {
      const yrMatch = slot.bulan.match(/\b(202[0-9]|203[0-9])\b/);
      if (yrMatch) {
        return Number(yrMatch[1]);
      }
    }
    return fallbackYear;
  };

  // Helper to extract canonical lowercase month name from a slot
  const getSlotMonthName = (slot?: { bulan?: string }): string => {
    if (!slot || !slot.bulan) return '';
    const clean = slot.bulan.trim().toLowerCase();
    const mMatch = clean.match(/(januari|februari|maret|april|mei|juni|juli|agustus|september|oktober|november|desember|jan|feb|mar|apr|jun|jul|agt|agu|sep|okt|nov|des)/i);
    if (mMatch) {
      return mMatch[1].toLowerCase();
    }
    return clean;
  };

  // Robust matching helpers for Month & Year across string/number/abbreviated/embedded formats
  const isMatchSlot = (b?: { bulan?: string; tahun?: number | string }, targetBulan?: string, targetTahun?: number | string): boolean => {
    if (!b || !b.bulan || !targetBulan) return false;
    const m1 = getSlotMonthName(b);
    const m2 = targetBulan.trim().toLowerCase();
    const monthMatches = m1 === m2 || (m1.slice(0, 3) === m2.slice(0, 3) && m1.length >= 3 && m2.length >= 3);
    if (!monthMatches) return false;
    const y1 = getSlotYear(b);
    const y2 = Number(targetTahun) || 2026;
    return y1 === y2;
  };
  ...
  const handleReceiptUpload = async (files: File[]) => {
    ...
    try {
      ...
      triggerCropper(compressed, 'free', 'Potong Bukti Pembayaran', (cropped) => {
        const newReceipt = { base64: cropped, name: firstFile.name };
  ...
  const [printDensity, setPrintDensity] = useState<'normal' | 'compact' | 'super-compact'>('super-compact');
  ...
  const [payDiskonType, setPayDiskonType] = useState<'nominal' | 'pembebasan'>('nominal');
  const [payDiskonNominal, setPayDiskonNominal] = useState<number>(0);
  const [payDiskonAlasan, setPayDiskonAlasan] = useState<string>('Pembebasan Lansia / Dhuafa / Yatim');
  const [payDiskonAlasanCustom, setPayDiskonAlasanCustom] = useState<string>('');

  const [payRombongHasDiskon, setPayRombongHasDiskon] = useState<boolean>(false);
  const [payRombongDiskonType, setPayRombongDiskonType] = useState<'nominal' | 'pembebasan'>('nominal');
  const [payRombongDiskonNominal, setPayRombongDiskonNominal] = useState<number>(0);
  const [payRombongDiskonAlasan, setPayRombongDiskonAlasan] = useState<string>('Pembebasan Lansia / Dhuafa / Yatim');
  const [payRombongDiskonAlasanCustom, setPayRombongDiskonAlasanCustom] = useState<string>('');

  // States for Discount & Fee Waiver in Warga & Rombong correction modals
  const [corrHasDiskon, setCorrHasDiskon] = useState<boolean>(false);
  const [corrDiskonType, setCorrDiskonType] = useState<'nominal' | 'pembebasan'>('nominal');
  const [corrDiskonNominal, setCorrDiskonNominal] = useState<number>(0);
  const [corrDiskonAlasan, setCorrDiskonAlasan] = useState<string>('Pembebasan Lansia / Dhuafa / Yatim');
  const [corrDiskonAlasanCustom, setCorrDiskonAlasanCustom] = useState<string>('');

  const [corrRombongHasDiskon, setCorrRombongHasDiskon] = useState<boolean>(false);
  const [corrRombongDiskonType, setCorrRombongDiskonType] = useState<'nominal' | 'pembebasan'>('nominal');
  const [corrRombongDiskonNominal, setCorrRombongDiskonNominal] = useState<number>(0);
  const [corrRombongDiskonAlasan, setCorrRombongDiskonAlasan] = useState<string>('Pembebasan Lansia / Dhuafa / Yatim');
  const [corrRombongDiskonAlasanCustom, setCorrRombongDiskonAlasanCustom] = useState<string>('');
  ...
  const isRombongMacet = (r: RombongBill, curYear: number, curMonth: string): boolean => {
    ...
    const unpaidCurrent = fullMonths.slice(0, monthIndex === -1 ? 0 : monthIndex).some(m => {
      const slot = r.iuranRombong.find(b => isMatchSlot(b, m, curYear));
  ...
  const isCitizenTx = (entry: LedgerEntry, citizenName: string, block: string, noRumah: string) => {
    ...
    const blok = (block || '').toLowerCase().trim();
    const num = (noRumah || '').toLowerCase().trim();
    
    if (blok && num) {
      const standardPair = `${blok}-${num}`;
      const noLeadingZero = `${blok}-${num.replace(/^0+/, '')}`;
      const withLeadingZero = `${blok}-${num.length === 1 ? '0' + num : num}`;
      const spacePair = `${blok} ${num}`;
      const slashPair = `${blok}/${num}`;
      
      if (
        desc.includes(standardPair) ||
        desc.includes(noLeadingZero) ||
        desc.includes(withLeadingZero) ||
        desc.includes(spacePair) ||
        desc.includes(slashPair)
      ) {
        return true;
      }
    }
    
    const cleanName = (citizenName || '').toLowerCase()
      .replace(/^(bapak|bpk|bp\.|ibu|bu\.|pak|mas|mbak|h\.|hj\.)\s+/i, '')
      .trim();
    
    if (cleanName.length >= 3 && desc.includes(cleanName)) {
      return true;
    }
    
    const parenNameMatch = (citizenName || '').match(/\(([^)]+)\)/);
    if (parenNameMatch) {
      const alias = parenNameMatch[1].toLowerCase().replace(/^(bapak|bpk|bp\.|ibu|bu\.|pak|mas|mbak)\s+/i, '').trim();
      if (alias.length >= 3 && desc.includes(alias)) {
        return true;
      }
    }
  ...
  // Helper to extract all billing/target years from a ledger entry
  const getLedgerEntryYears = (entry: LedgerEntry): number[] => {
    const years = new Set<number>();
    if (entry.tahun && !isNaN(Number(entry.tahun)) && Number(entry.tahun) > 0) {
      years.add(Number(entry.tahun));
    }
    const desc = (entry.deskripsi || '').toLowerCase();
    const explicitBulan = (entry.bulan || '').toLowerCase();
    
    // Find all 4-digit years in description or explicit bulan e.g. 2024, 2025, 2026, 2027
    const matchesDesc = desc.match(/\b(202[0-9]|203[0-9])\b/g);
    if (matchesDesc) {
      matchesDesc.forEach(yStr => years.add(Number(yStr)));
    }
    const matchesBulan = explicitBulan.match(/\b(202[0-9]|203[0-9])\b/g);
    if (matchesBulan) {
      matchesBulan.forEach(yStr => years.add(Number(yStr)));
    }
    
    // Fallback to entry.tanggal year ONLY if no explicit year was found in description/tahun/bulan
    if (years.size === 0 && entry.tanggal) {
      const txYear = new Date(entry.tanggal).getFullYear();
      if (!isNaN(txYear)) years.add(txYear);
    }
    if (years.size === 0) {
      years.add(2026);
    }
  ...
  const filterLedgerForWarga = (citizen: WargaBill, targetYear: number) => {
    return ledger.filter(entry => {
      ...
      const entryYears = getLedgerEntryYears(entry);
      return entryYears.includes(targetYear);
    });
  };
  ...
  const filterLedgerForRombong = (rtLapak: RombongBill, targetYear: number) => {
    return ledger.filter(entry => {
      const desc = (entry.deskripsi || '').toLowerCase();
      const owner = (rtLapak.namaPemilik || '').toLowerCase().trim();
      const cleanOwner = owner.split('(')[0].trim();
      const lapakNo = (rtLapak.noLapak || '').toLowerCase().trim();
      const cleanLokasi = (rtLapak.lokasi || (rtLapak as any).namaUsaha || '').toLowerCase().trim();
      
      const matchesOwner = (owner.length >= 3 && desc.includes(owner)) || 
                           (cleanOwner.length >= 3 && desc.includes(cleanOwner)) || 
                           (lapakNo.length > 0 && desc.includes(lapakNo)) ||
                           (cleanLokasi.length >= 3 && desc.includes(cleanLokasi));
  ...
  const getWargaArrearsBeforeYear = (w: WargaBill, targetYear: number): number => {
    ...
    priorYears.forEach(yr => {
      defaultMonths.forEach(m => {
        const slot = w.iuranRT.find(b => isMatchSlot(b, m, yr));
        const isLunas = slot ? slot.lunas : false;
        if (!isLunas) {
  ...
  const getRombongArrearsBeforeYear = (r: RombongBill, targetYear: number): number => {
    ...
    priorYears.forEach(yr => {
      defaultMonths.forEach(m => {
        const slot = r.iuranRombong.find(b => isMatchSlot(b, m, yr));
        const isLunas = slot ? slot.lunas : false;
        if (!isLunas) {
  ...
  const printWargaInvoice = (w: WargaBill, targetYear: number) => {
    ...
    defaultMonths.forEach(m => {
      const slot = w.iuranRT.find(b => isMatchSlot(b, m, targetYear));
      const isLunas = slot ? slot.lunas : false;
  ...
  const printRombongInvoice = (r: RombongBill, targetYear: number) => {
    ...
    defaultMonths.forEach(m => {
      const slot = r.iuranRombong.find(b => isMatchSlot(b, m, targetYear));
      const isLunas = slot ? slot.lunas : false;
      const isDue = isMonthDue(m, targetYear);
      const nominal = isLunas 
  ...
  // Comprehensive Reconcile & Synchronize citizen/stall billing status directly from Ledger (Buku Kas)
  const syncPaymentsFromLedger = () => {
    if (!ledger || ledger.length === 0) {
      alert('Tidak ada data transaksi Buku Kas untuk disinkronkan.');
      return;
    }
    ...
    const monthKeys: { [key: string]: string } = {
      ...
      'juni': 'Juni', 'jun': 'Juni',
      'juli': 'Juli', 'jul': 'Juli',
      'agustus': 'Agustus', 'agt': 'Agustus', 'agu': 'Agustus', 'aug': 'Agustus',
    ...
    // Helper to parse all period pairs (bulan, tahun) from a ledger transaction
    const parsePeriodsFromEntry = (entry: LedgerEntry): { bulan: string; tahun: number }[] => {
      const desc = (entry.deskripsi || '').toLowerCase();
      const explicitBulan = (entry.bulan || '').toLowerCase();
      
      // Determine default base year from entry
      let entryBaseYear = 2026;
      if (entry.tahun && !isNaN(Number(entry.tahun)) && Number(entry.tahun) > 0) {
        entryBaseYear = Number(entry.tahun);
      } else {
        const yrInDesc = desc.match(/\b(202[0-9]|203[0-9])\b/);
        if (yrInDesc) {
          entryBaseYear = Number(yrInDesc[1]);
        } else if (entry.tanggal) {
          const dtYear = new Date(entry.tanggal).getFullYear();
          if (!isNaN(dtYear) && dtYear > 0) entryBaseYear = dtYear;
        }
      }
      
      const results: { bulan: string; tahun: number }[] = [];
      const addedSet = new Set<string>();

      const addPeriod = (mName: string, yr: number) => {
        const key = `${mName.toLowerCase()}-${yr}`;
        if (!addedSet.has(key)) {
          addedSet.add(key);
          results.push({ bulan: mName, tahun: yr });
        }
      };

      // 1. Check parenthesized collective items: e.g. "(Januari 2026, Februari 2026)", "(Oktober 2025, November 2025)", "(Januari, Februari, Maret 2025)"
      const parenMatches = desc.match(/\(([^)]+)\)/g);
      if (parenMatches) {
        parenMatches.forEach(parenStr => {
          const insideParen = parenStr.replace(/[\(\)]/g, '');
          const parenYearMatch = insideParen.match(/\b(202[0-9]|203[0-9])\b/);
          const parenDefaultYear = parenYearMatch ? Number(parenYearMatch[1]) : entryBaseYear;

          const monthYearRegex = /(januari|februari|maret|april|mei|juni|juli|agustus|september|oktober|november|desember|jan|feb|mar|apr|jun|jul|agt|agu|sep|okt|nov|des)\s*(\d{4})?/gi;
          let m;
          while ((m = monthYearRegex.exec(insideParen)) !== null) {
            const canonicalMonth = monthKeys[m[1].toLowerCase()];
            if (canonicalMonth) {
              const itemYear = m[2] ? Number(m[2]) : parenDefaultYear;
              addPeriod(canonicalMonth, itemYear);
            }
          }
        });
      }

      // 2. Check range patterns: e.g. "Mei 2025 s/d Juli 2025", "Januari - Juni 2026", "Okt 2025 sd Des 2025", "Nov 2025 - Feb 2026"
      const rangeRegex = /(januari|februari|maret|april|mei|juni|juli|agustus|september|oktober|november|desember|jan|feb|mar|apr|jun|jul|agt|agu|sep|okt|nov|des)\s*(\d{4})?\s*(?:s\/d|sd|sampai|-|–|to)\s*(januari|februari|maret|april|mei|juni|juli|agustus|september|oktober|november|desember|jan|feb|mar|apr|jun|jul|agt|agu|sep|okt|nov|des)\s*(\d{4})?/gi;
      let rangeMatch;
      while ((rangeMatch = rangeRegex.exec(desc)) !== null) {
        const startM = monthKeys[rangeMatch[1].toLowerCase()];
        const endM = monthKeys[rangeMatch[3].toLowerCase()];
        const startYr = rangeMatch[2] ? Number(rangeMatch[2]) : (rangeMatch[4] ? Number(rangeMatch[4]) : entryBaseYear);
        const endYr = rangeMatch[4] ? Number(rangeMatch[4]) : startYr;
        
        if (startM && endM) {
          const startIdx = fullMonths.indexOf(startM);
          const endIdx = fullMonths.indexOf(endM);
          
          if (startYr === endYr && startIdx !== -1 && endIdx !== -1 && startIdx <= endIdx) {
            for (let i = startIdx; i <= endIdx; i++) {
              addPeriod(fullMonths[i], startYr);
            }
          } else if (startYr < endYr && startIdx !== -1 && endIdx !== -1) {
            // Cross-year range (e.g. Nov 2025 to Feb 2026)
            for (let i = startIdx; i < 12; i++) {
              addPeriod(fullMonths[i], startYr);
            }
            for (let i = 0; i <= endIdx; i++) {
              addPeriod(fullMonths[i], endYr);
            }
          }
        }
      }

      // 3. Check individual month-year patterns in description, e.g. "Bulan Oktober 2025", "Bulan Juni 2026", "Iuran 2025 Bln Okt"
      const singleRegex = /(?:bulan|bln|periode)?\s*(januari|februari|maret|april|mei|juni|juli|agustus|september|oktober|november|desember|jan|feb|mar|apr|jun|jul|agt|agu|sep|okt|nov|des)\s*(?:tahun|thn|th)?\s*(\d{4})?/gi;
      let singleMatch;
      while ((singleMatch = singleRegex.exec(desc)) !== null) {
        const canonicalMonth = monthKeys[singleMatch[1].toLowerCase()];
        if (canonicalMonth) {
          let matchedYear = singleMatch[2] ? Number(singleMatch[2]) : null;
          if (!matchedYear) {
            // Check if there is a year anywhere in the same clause or description
            const yearInDesc = desc.match(/\b(202[0-9]|203[0-9])\b/);
            matchedYear = yearInDesc ? Number(yearInDesc[1]) : entryBaseYear;
          }
          addPeriod(canonicalMonth, matchedYear);
        }
      }

      // 4. Check explicit entry.bulan if provided
      if (explicitBulan) {
        const expParts = explicitBulan.split(/[,;\/]+/);
        expParts.forEach(part => {
          const p = part.trim();
          const mMatch = p.match(/(januari|februari|maret|april|mei|juni|juli|agustus|september|oktober|november|desember|jan|feb|mar|apr|jun|jul|agt|agu|sep|okt|nov|des)/i);
          const yMatch = p.match(/\b(202[0-9]|203[0-9])\b/);
          if (mMatch) {
            const canonical = monthKeys[mMatch[1].toLowerCase()];
            if (canonical) {
              const yr = yMatch ? Number(yMatch[1]) : entryBaseYear;
              addPeriod(canonical, yr);
            }
          }
        });
      }

      // Fallback: If no periods were extracted, check if entry has bulan
      if (results.length === 0 && entry.bulan && monthKeys[entry.bulan.toLowerCase()]) {
        addPeriod(monthKeys[entry.bulan.toLowerCase()], entryBaseYear);
      }

      return results;
    };

    const isWargaMatchingEntry = (w: WargaBill, desc: string): boolean => {
      const cleanDesc = desc.toLowerCase();
      const blok = (w.blok || '').toLowerCase().trim();
      const noRumah = (w.noRumah || '').toLowerCase().trim();
      
      if (blok && noRumah) {
        const standardPair = `${blok}-${noRumah}`;
        const noLeadingZero = `${blok}-${noRumah.replace(/^0+/, '')}`;
        const withLeadingZero = `${blok}-${noRumah.length === 1 ? '0' + noRumah : noRumah}`;
        const spacePair = `${blok} ${noRumah}`;
        const slashPair = `${blok}/${noRumah}`;
        
        if (
          cleanDesc.includes(standardPair) ||
          cleanDesc.includes(noLeadingZero) ||
          cleanDesc.includes(withLeadingZero) ||
          cleanDesc.includes(spacePair) ||
          cleanDesc.includes(slashPair)
        ) {
          return true;
        }
      }

      const cleanName = (w.nama || '').toLowerCase().trim();
      if (cleanName.length >= 3) {
        if (cleanDesc.includes(cleanName)) return true;
        
        const parts = cleanName.split(/[\(\)\/\-]+/).map(p => p.trim()).filter(p => p.length >= 3);
        for (const part of parts) {
          const pureName = part.replace(/\b(bpk|bp|ibu|bu|pak|ustad|haji|h\.)\b/g, '').trim();
          if (pureName.length >= 3 && cleanDesc.includes(pureName)) {
            return true;
          }
        }
      }
    ...
    const isRombongMatchingEntry = (r: RombongBill, desc: string): boolean => {
      const cleanDesc = desc.toLowerCase();
      const noLapak = (r.noLapak || '').toLowerCase().trim();
      if (noLapak.length >= 1) {
        if (cleanDesc.includes(noLapak) || cleanDesc.includes(`lapak ${noLapak}`) || cleanDesc.includes(`rombong ${noLapak}`)) {
          return true;
        }
      }

      const cleanPemilik = (r.namaPemilik || '').toLowerCase().trim();
      if (cleanPemilik.length >= 3 && cleanDesc.includes(cleanPemilik)) {
        return true;
      }

      const cleanLokasi = (r.lokasi || (r as any).namaUsaha || '').toLowerCase().trim();
      if (cleanLokasi.length >= 3 && cleanDesc.includes(cleanLokasi)) {
        return true;
      }
    ...
    let matchedWargaCount = 0;
    let matchedRombongCount = 0;

    // 1. Process Warga List Reconciliation
    const workingWargaList = wargaList.map(w => {
      const iuranRT = Array.isArray(w.iuranRT) ? [...w.iuranRT] : [];
      let wargaModified = false;

      ledger.forEach(entry => {
        if (entry.tipe !== 'pemasukan') return;
        const desc = (entry.deskripsi || '').toLowerCase();
        
        if (!isWargaMatchingEntry(w, desc)) return;

        const periods = parsePeriodsFromEntry(entry);
        if (periods.length === 0) return;

        const perMonthNominal = entry.jumlah && periods.length > 0 ? Math.round(entry.jumlah / periods.length) : 35000;
        const paymentDate = entry.tanggal || new Date().toISOString().split('T')[0];
        const paymentTime = (entry as any).jam || '12:00';

        periods.forEach(p => {
          const slotIdx = iuranRT.findIndex(b => isMatchSlot(b, p.bulan, p.tahun));
          if (slotIdx > -1) {
            const currentSlot = iuranRT[slotIdx];
            if (!currentSlot.lunas) {
              iuranRT[slotIdx] = {
                ...currentSlot,
                lunas: true,
                nominal: perMonthNominal || currentSlot.nominal || getDefaultRtRate(p.tahun, p.bulan, rateRT),
                tahun: p.tahun,
                tanggalBayar: currentSlot.tanggalBayar || paymentDate,
                jamBayar: currentSlot.jamBayar || paymentTime,
                catatan: currentSlot.catatan || (periods.length > 1 ? 'Kolektif Buku Kas' : undefined),
                manualKoreksi: true
              };
              wargaModified = true;
              matchedWargaCount++;
            ...
            iuranRT.push({
              bulan: p.bulan,
              lunas: true,
              nominal: perMonthNominal || getDefaultRtRate(p.tahun, p.bulan, rateRT),
              tahun: p.tahun,
              tanggalBayar: paymentDate,
              jamBayar: paymentTime,
              catatan: periods.length > 1 ? 'Kolektif Buku Kas' : undefined,
              manualKoreksi: true
            });
            wargaModified = true;
            matchedWargaCount++;
    ...
    // 2. Process Rombong / Lapak List Reconciliation
    const workingRombongList = (rombongList || []).map(r => {
      const iuranRombong = Array.isArray(r.iuranRombong) ? [...r.iuranRombong] : [];
      let rombongModified = false;

      ledger.forEach(entry => {
        if (entry.tipe !== 'pemasukan') return;
        const desc = (entry.deskripsi || '').toLowerCase();
        
        if (!isRombongMatchingEntry(r, desc)) return;

        const periods = parsePeriodsFromEntry(entry);
        if (periods.length === 0) return;

        const perMonthNominal = entry.jumlah && periods.length > 0 ? Math.round(entry.jumlah / periods.length) : 50000;
        const paymentDate = entry.tanggal || new Date().toISOString().split('T')[0];
        const paymentTime = (entry as any).jam || '12:00';

        periods.forEach(p => {
          const slotIdx = iuranRombong.findIndex(b => isMatchSlot(b, p.bulan, p.tahun));
          if (slotIdx > -1) {
            const currentSlot = iuranRombong[slotIdx];
            if (!currentSlot.lunas) {
              iuranRombong[slotIdx] = {
                ...currentSlot,
                lunas: true,
                nominal: perMonthNominal || currentSlot.nominal || getDefaultRombongRate(p.tahun, p.bulan, rateRombong),
                tahun: p.tahun,
                tanggalBayar: currentSlot.tanggalBayar || paymentDate,
                jamBayar: currentSlot.jamBayar || paymentTime,
                catatan: currentSlot.catatan || (periods.length > 1 ? 'Kolektif Buku Kas' : undefined),
            ...
            iuranRombong.push({
              bulan: p.bulan,
              lunas: true,
              nominal: perMonthNominal || getDefaultRombongRate(p.tahun, p.bulan, rateRombong),
              tahun: p.tahun,
              tanggalBayar: paymentDate,
              jamBayar: paymentTime,
              catatan: periods.length > 1 ? 'Kolektif Buku Kas' : undefined,
    ...
    if (matchedWargaCount > 0) {
      updateWargaList(workingWargaList);
      if (selectedWargaHistory) {
        const refreshedW = workingWargaList.find(w => w.id === selectedWargaHistory.id);
        if (refreshedW) setSelectedWargaHistory(refreshedW);
      }
    }

    if (matchedRombongCount > 0 && updateRombongList) {
      updateRombongList(workingRombongList);
      if (selectedRombongHistory) {
        const refreshedR = workingRombongList.find(r => r.id === selectedRombongHistory.id);
        if (refreshedR) setSelectedRombongHistory(refreshedR);
      }
    }

    const totalSynced = matchedWargaCount + matchedRombongCount;
    if (totalSynced > 0) {
      alert(`Sinkronisasi Selesai: Berhasil menyinkronkan & melunaskan ${matchedWargaCount} tagihan iuran RT warga dan ${matchedRombongCount} tagihan sewa rombong dari transaksi Buku Kas (termasuk tagihan kolektif & multi-bulan)!`);
    } else {
      alert('Semua data tagihan warga dan rombong telah sinkron dan sesuai dengan Buku Kas.');
    }
  };
  ...
  const startEditFamilyMember = (member: any) => {
    ...
    setTempMember({
      ...
      nik: member.nik || '',
      noHape: member.noHape || '',
      fotoBase64: member.fotoBase64,
      fotoNamaFile: member.fotoNamaFile
    });
  };
  ...
  const addFamilyMemberToList = () => {
    ...
    if (editingMemberId) {
      ...
      if (editingWarga) {
        ...
        setEditingWarga({
          ...editingWarga,
          anggotaKeluarga: existingMembers.map(m => m.id === editingMemberId ? {
            ...m,
            ...
            fotoBase64: tempMember.fotoBase64,
            fotoNamaFile: tempMember.fotoNamaFile
          } : m)
        });
      } else {
  ...
  const getTerbilang = (nilai: number): string => {
    ...
    if (nilai < 0) return "Minus " + getTerbilang(Math.abs(nilai));
    let temp = "";
    if (nilai < 12) {
      temp = " " + bilangan[nilai];
    } else if (nilai < 20) {
  ...
  const drawReceiptOnCanvas = (receiptInfo: {
    ...
    if (receiptInfo.isPembebasan) {
      fields.push({
        ...
        value: `PEMBEBASAN TOTAL 100% (${receiptInfo.alasanDiskon || 'Kebijakan Musyawarah RT'})`,
        isHighlight: true
      });
    } else if (receiptInfo.diskon && receiptInfo.diskon > 0) {
      fields.push({
        label: 'POTONGAN / DISKON',
        value: `Rp ${receiptInfo.diskon.toLocaleString('id-ID')} (${receiptInfo.alasanDiskon || 'Kebijakan Musyawarah RT'})`,
        isHighlight: false
      });
    }
  ...
  const openReceiptReprint = (
    item: WargaBill | RombongBill,
    tipe: 'warga' | 'rombong',
    slot: any,
    year: number
  ) => {
    let matchedKas = tipe === 'warga' ? 'rtTunai' : 'rombongTunai';
    let matchedPetugas = 'Petugas RT';

    if (tipe === 'warga') {
      const citizen = item as WargaBill;
      const matchingTxs = filterLedgerForWarga(citizen, year);
      const monthTx = matchingTxs.find(tx => {
        const desc = (tx.deskripsi || '').toLowerCase();
        return desc.includes(slot.bulan.toLowerCase()) && (desc.includes('iuran rt') || desc.includes('iuranrt'));
      });
      if (monthTx) {
        matchedKas = monthTx.sumberKas || 'rtTunai';
        matchedPetugas = monthTx.petugas || 'Petugas RT';
      }
    } else {
      const rombong = item as RombongBill;
      const matchingTxs = filterLedgerForRombong(rombong, year);
      const monthTx = matchingTxs.find(tx => {
        const desc = (tx.deskripsi || '').toLowerCase();
        return desc.includes(slot.bulan.toLowerCase()) && desc.includes('rombong');
      });
      if (monthTx) {
        matchedKas = monthTx.sumberKas || 'rombongTunai';
        matchedPetugas = monthTx.petugas || 'Petugas RT';
      }
    }
    ...
    setReceiptSuccessInfo({
      id: item.id,
      nama: tipe === 'warga' ? (item as WargaBill).nama : (item as RombongBill).namaPemilik,
      tipe: tipe,
      blok: tipe === 'warga' ? (item as WargaBill).blok : undefined,
      noRumah: tipe === 'warga' ? (item as WargaBill).noRumah : undefined,
      noLapak: tipe === 'rombong' ? (item as RombongBill).noLapak : undefined,
      noWa: item.noWa || '',
      category: tipe === 'warga' ? 'Iuran RT' : 'Iuran Rombong',
      bulan: slot.bulan,
      tahun: year,
      nominal: slot.nominal,
      tanggalBayar: slot.tanggalBayar || new Date().toISOString().split('T')[0],
      jamBayar: slot.jamBayar || '00:00',
      kasPenerima: matchedKas,
      petugas: matchedPetugas,
      catatan: slot.catatan ? `Catatan: ${slot.catatan}` : ''
    });
  };
  ...
  const openPaymentModal = (
    ...
    if (warga.isBebasIuranPermanen) {
      ...
      setPayDiskonNominal(warga.defaultDiskon);
  ...
  const openRombongPaymentModal = (
    ...
    setPayRombongHasDiskon(false);
    setPayRombongDiskonType('nominal');
    setPayRombongDiskonNominal(0);
    setPayRombongDiskonAlasan('Pembebasan Lansia / Dhuafa / Yatim');
    setPayRombongDiskonAlasanCustom('');
  ...
  const processPayment = () => {
    ...
    if (payHasDiskon) {
      ...
      if (payDiskonType === 'pembebasan') {
        ...
        finalDiskon = Math.min(nominal, payDiskonNominal);
        netNominal = Math.max(0, nominal - finalDiskon);
        if (netNominal === 0) isPembebasan = true;
      }
    }

    const updatedWargaList = wargaList.map(w => {
      if (w.id === warga.id) {
        const index = w[billingType].findIndex(b => isMatchSlot(b, bulan, tahun));
        let updatedBillings = [...w[billingType]];
        if (index > -1) {
          updatedBillings = updatedBillings.map(b => {
            if (isMatchSlot(b, bulan, tahun)) {
              return { 
                ...b, 
                lunas: true, 
                nominal: netNominal,
                diskon: finalDiskon > 0 ? finalDiskon : undefined,
                isPembebasan: isPembebasan || undefined,
                alasanDiskon: alasanDiskon || undefined,
                tanggalBayar: paymentDate, 
                jamBayar: paymentTime,
    ...
    if (isPembebasan) {
      ledgerDesc += ` [PEMBEBASAN TOTAL 100% - ${alasanDiskon}]`;
  ...
  const processBatchPayment = () => {
    ...
    const updatedWargaList = wargaList.map(w => {
      if (w.id === warga.id) {
        ...
        items.forEach(item => {
          const index = updatedBillings.findIndex(b => isMatchSlot(b, item.bulan, item.tahun));
  ...
  const processRombongPayment = () => {
    ...
    const updatedRombongList = rombongList.map(r => {
      if (r.id === rombong.id) {
        const index = r[billingType].findIndex(b => isMatchSlot(b, bulan, tahun));
  ...
  const openCorrectionModal = (
    ...
    const slot = warga.iuranRT.find(b => isMatchSlot(b, bulan, tahun));
  ...
  const openRombongCorrectionModal = (
    ...
    const slot = rombong.iuranRombong.find(b => isMatchSlot(b, bulan, tahun));
    const existingDate = slot?.tanggalBayar || new Date().toISOString().split('T')[0];
    const existingTime = slot?.jamBayar || (() => {
      const now = new Date();
      return `${String(now.getHours()).padStart(2, '0')}:${String(now.getMinutes()).padStart(2, '0')}`;
  ...
  const saveCorrection = () => {
      ...
      const updatedWargaList = wargaList.map(w => {
        ...
        if (w.id === warga.id) {
          const updatedBillings = w[billingType].map(b => {
            const isMatch = isMatchSlot(b, bulan, initialTahun);
            if (isMatch) {
        ...
        if (w.id === corrTransferTargetWargaId) {
          const index = w[billingType].findIndex(b => isMatchSlot(b, bulan, corrTahun));
          let updatedBillings = [...w[billingType]];
          if (index > -1) {
            updatedBillings = updatedBillings.map(b => {
              if (isMatchSlot(b, bulan, corrTahun)) {
                return { ...b, lunas: true, nominal: corrNominal, tahun: corrTahun, tanggalBayar: corrPaymentDate, jamBayar: corrPaymentTime, catatan: corrCatatan, manualKoreksi: true };
              }
              return b;
            });
          } else {
    ...
    const slot = warga.iuranRT.find(b => isMatchSlot(b, bulan, initialTahun));
    const originallyNoCashFlow = slot?.noCashFlow || false;
    const isNoCashFlowNow = corrNoCashFlow || originallyNoCashFlow;

    let finalCorrDiskon = 0;
    let isCorrPembebasan = false;
    let alasanCorrDiskon = '';
    let finalCorrNetNominal = corrNominal;

    if (corrHasDiskon) {
      alasanCorrDiskon = corrDiskonAlasan === 'Lainnya (Tulis Sendiri)' ? corrDiskonAlasanCustom : corrDiskonAlasan;
      if (corrDiskonType === 'pembebasan') {
        isCorrPembebasan = true;
        finalCorrDiskon = corrNominal;
        finalCorrNetNominal = 0;
      } else {
        finalCorrDiskon = Math.min(corrNominal, corrDiskonNominal);
        finalCorrNetNominal = Math.max(0, corrNominal - finalCorrDiskon);
        if (finalCorrNetNominal === 0) isCorrPembebasan = true;
      }
    }

    const updatedWargaList = wargaList.map(w => {
      if (w.id === warga.id) {
        const index = w[billingType].findIndex(b => isMatchSlot(b, bulan, initialTahun));
        let updatedBillings = [...w[billingType]];
            ...
            updatedBillings = updatedBillings.map(b => {
              ...
              if (isMatch) {
                return { 
                  ...b, 
                  lunas: true, 
                  nominal: finalCorrNetNominal, 
                  diskon: finalCorrDiskon > 0 ? finalCorrDiskon : undefined,
                  isPembebasan: isCorrPembebasan || undefined,
                  alasanDiskon: alasanCorrDiskon || undefined,
                  tahun: corrTahun, 
                  tanggalBayar: corrPaymentDate, 
                  jamBayar: corrPaymentTime, 
  ...
  const saveRombongCorrection = () => {
    ...
    if (corrTransferTargetRombongId) {
      ...
      const updatedRombongList = rombongList.map(r => {
        ...
        if (r.id === corrTransferTargetRombongId) {
          const index = r[billingType].findIndex(b => isMatchSlot(b, bulan, corrRombongTahun));
          let updatedBillings = [...r[billingType]];
          if (index > -1) {
            updatedBillings = updatedBillings.map(b => {
              if (isMatchSlot(b, bulan, corrRombongTahun)) {
                return { ...b, lunas: true, nominal: corrRombongNominal, tahun: corrRombongTahun, tanggalBayar: corrPaymentDate, jamBayar: corrPaymentTime, catatan: corrRombongCatatan, manualKoreksi: true };
              }
              return b;
            });
          } else {
    ...
    const slot = rombong.iuranRombong.find(b => isMatchSlot(b, bulan, initialTahun));
    const originallyNoCashFlow = slot?.noCashFlow || false;
    const isNoCashFlowNow = corrRombongNoCashFlow || originallyNoCashFlow;

    const updatedRombongList = rombongList.map(r => {
      if (r.id === rombong.id) {
        const index = r[billingType].findIndex(b => isMatchSlot(b, bulan, initialTahun));
        let updatedBillings = [...r[billingType]];
  ...
  let filteredWarga = safeWargaList.filter(w => w && !w.isDeleted).filter(w => {
    ...
    const hasOutstanding = defaultMonths.some(m => {
      const slot = iuranRT.find(b => isMatchSlot(b, m, selectedBillingYear));
  ...
  let filteredRombong = safeRombongList.filter(r => r && !r.isDeleted).filter(r => {
    ...
    const iuranRombong = Array.isArray(r.iuranRombong) ? r.iuranRombong : [];
    const hasOutstanding = defaultMonths.some(m => {
      const slot = iuranRombong.find(b => isMatchSlot(b, m, selectedBillingYear));
      return !slot || !slot.lunas;
    });
  ...
  const reportWarga = safeWargaList.filter(w => {
    ...
    if (w.isDeleted) {
      const hasAnyPaidInYear = iuranRT.some(b => 
        b && b.lunas && getSlotYear(b, selectedBillingYear) === Number(selectedBillingYear)
      );
  ...
  const outstandingWargaBillsCount = safeWargaList.filter(w => w && !w.isDeleted && (!w.statusKeaktifan || w.statusKeaktifan === 'aktif')).reduce((acc, w) => {
    ...
    priorYears.forEach(yr => {
      defaultMonths.forEach(m => {
        const slot = iuranRT.find(b => isMatchSlot(b, m, yr));
        if (!slot || !slot.lunas) {
          count++;
        }
      });
  ...
  const outstandingRombongBillsCount = safeRombongList.filter(r => r && !r.isDeleted && (!r.statusKeaktifan || r.statusKeaktifan === 'aktif')).reduce((acc, r) => {
    ...
    priorYears.forEach(yr => {
      defaultMonths.forEach(m => {
        const slot = iuranRombong.find(b => isMatchSlot(b, m, yr));
        if (!slot || !slot.lunas) {
          count++;
        }
      });
  ...
  const handlePrintWargaAnnualBook = (warga: WargaBill, targetYear: number) => {
    const totalPaidMonths = fullMonths.filter(m => {
      const slot = warga.iuranRT.find(b => isMatchSlot(b, m, targetYear));
      return slot && slot.lunas;
    }).length;
    ...
    const totalPaidAmount = warga.iuranRT
      .filter(b => b.lunas && getSlotYear(b, targetYear) === Number(targetYear))
  ...
  const handlePrintRombongAnnualBook = (rombong: RombongBill, targetYear: number) => {
    const totalPaidMonths = fullMonths.filter(m => {
      const slot = rombong.iuranRombong.find(b => isMatchSlot(b, m, targetYear));
      return slot && slot.lunas;
    }).length;
  ...
  const getWhatsAppMessageText = (warga: WargaBill) => {
    ...
    defaultMonths.forEach(m => {
      const slot = warga.iuranRT.find(b => isMatchSlot(b, m, selectedBillingYear));
      if ((!slot || !slot.lunas) && isMonthDue(m, selectedBillingYear)) {
        unpaidRT.push(m);
      }
    });
  ...
  const handleOpenWhatsAppLink = (warga: WargaBill) => {
    ...
    const finalUrl = formattedPhone
      ? `https://api.whatsapp.com/send?phone=${formattedPhone}&text=${encodeURIComponent(rawMsg)}`
      : `https://api.whatsapp.com/send?text=${encodeURIComponent(rawMsg)}`;
  ...
  const getWhatsAppRombongMessageText = (rombong: RombongBill) => {
    ...
    defaultMonths.forEach(m => {
      const slot = rombong.iuranRombong.find(b => isMatchSlot(b, m, selectedBillingYear));
      if ((!slot || !slot.lunas) && isMonthDue(m, selectedBillingYear)) {
          ...
          <div className="flex flex-col xl:flex-row gap-3 items-stretch xl:items-center justify-between">
            ...
            <div className="w-full xl:w-auto flex flex-col md:flex-row items-stretch xl:items-center gap-2 shrink-0">
              {isLoggedIn && (currentUser?.role === 'admin' || currentUser?.role === 'bendahara') && (
                <button
                  type="button"
                  onClick={syncPaymentsFromLedger}
                  className="bg-emerald-50 hover:bg-emerald-100 text-emerald-800 font-extrabold px-3 py-2 rounded-lg transition duration-200 text-xs whitespace-nowrap cursor-pointer flex items-center justify-center gap-1 border border-emerald-200 active:scale-95 shadow-xs"
                  title="Sinkronkan dan tandai lunas otomatis semua tagihan warga berdasarkan transaksi Buku Kas yang ada"
                >
          ...
          <form onSubmit={handleAddWarga} className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    onChange={async (e) => {
                      ...
                      if (file) {
                        try {
                          ...
                          triggerCropper(base64, 'free', 'Potong Foto KTP', (cropped) => {
                            setNewWarga(prev => ({
                              ...prev,
                              ktpBase64: cropped,
                  ...
                  <input 
                    ...
                    onChange={async (e) => {
                      ...
                      if (file) {
                        try {
                          const base64 = await compressImage(file);
                          triggerCropper(base64, 'free', 'Potong Foto KK', (cropped) => {
                            setNewWarga(prev => ({
                              ...prev,
                              kkBase64: cropped,
            ...
            <div className="md:col-span-3 bg-purple-50/50 border border-purple-200/80 rounded-xl p-4 space-y-3">
              ...
              <div className="md:col-span-3 border-t border-slate-100 pt-4 mt-2">
                  ...
                  {(!newWarga.anggotaKeluarga || newWarga.anggotaKeluarga.length === 0) ? (
                    ...
                    <div className="max-h-[180px] overflow-y-auto space-y-1.5 pr-1">
                      {newWarga.anggotaKeluarga.map((member, i) => (
                        <div key={member.id || i} className={`border rounded-xl p-2.5 flex justify-between items-center text-xs gap-3 transition ${editingMemberId === member.id ? 'bg-sky-50/50 border-sky-300 ring-1 ring-sky-300' : 'bg-slate-50 border-slate-150'}`}>
                          <div className="flex items-center gap-2.5 min-w-0">
                            {member.fotoBase64 ? (
                              <img src={member.fotoBase64} alt={member.nama} className="w-9 h-9 rounded-full object-cover border border-slate-200 shrink-0" referrerPolicy="no-referrer" />
                            ) : (
                            ...
                            <div className="min-w-0">
                              <div className="font-extrabold text-slate-850 truncate">{member.nama}</div>
                          ...
                          <div className="flex items-center gap-1.5 shrink-0">
                            <button
                              ...
                              className="p-1 px-1.5 bg-sky-50 hover:bg-sky-100 border border-sky-200 text-sky-650 rounded-lg cursor-pointer transition active:scale-95"
                              title="Ubah data anggota keluarga"
                            >
                ...
                {/* Form inline to add/edit a member */}
                <div className="bg-slate-50/50 border border-slate-200 rounded-2xl p-4 space-y-3">
                  <div className="text-[11px] font-extrabold text-slate-550 tracking-wider uppercase flex items-center gap-1">
                    ...
                    {editingMemberId ? 'Ubah Anggota Keluarga' : 'Tambah Anggota Keluarga'}
                  </div>
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-slate-600 mb-1">Foto Anggota (Opsional)</label>
                    <div className="flex items-center gap-3">
                      {tempMember.fotoBase64 ? (
                        <div className="relative w-12 h-12 rounded-full border border-slate-200 overflow-hidden shrink-0">
                          <img src={tempMember.fotoBase64} alt="Preview" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                          <button
                            type="button"
                            onClick={() => setTempMember({ ...tempMember, fotoBase64: undefined, fotoNamaFile: undefined })}
                            className="absolute inset-0 bg-black/50 flex items-center justify-center text-white opacity-0 hover:opacity-100 transition duration-150 cursor-pointer text-[9px] font-bold"
                          >
                            Hapus
                          </button>
                        </div>
                      ) : (
                        <div className="w-12 h-12 rounded-full border-2 border-dashed border-slate-200 flex items-center justify-center shrink-0 text-slate-400">
                          <Camera className="w-5 h-5" />
                        </div>
                      )}
                      <div className="flex-1 min-w-0">
                        <label className="inline-flex items-center px-2.5 py-1.5 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 rounded-lg text-xs font-semibold cursor-pointer transition">
                          <Upload className="w-3.5 h-3.5 mr-1 text-slate-500" />
                          {tempMember.fotoBase64 ? 'Ganti Foto' : 'Unggah Foto'}
                          <input
                            type="file"
                            accept="image/*"
                            onChange={async (e) => {
                              ...
                              if (file) {
                                try {
                                  const base64 = await compressImage(file);
                                  triggerCropper(base64, '1:1', 'Potong Foto Anggota Keluarga', (cropped) => {
                                    setTempMember(prev => ({
                                      ...prev,
                                      fotoBase64: cropped,
                                  ...
                                  alert('Gagal memproses gambar anggota.');
                                }
                              }
                            }}
                            className="hidden"
                        ...
                        {tempMember.fotoNamaFile && (
                          <span className="text-[10px] text-slate-500 font-semibold ml-2 block truncate max-w-[200px]" title={tempMember.fotoNamaFile}>
                            {tempMember.fotoNamaFile}
                          </span>
                        )}
                      </div>
                  ...
                  <div className="flex justify-end gap-2 pt-1">
                    ...
                    >
                      ...
                      {editingMemberId ? 'Simpan Perubahan' : 'Sematkan Anggota'}
                    </button>
                  </div>
                </div>
              </div>
                    ...
                    <input 
                      ...
                      onChange={async (e) => {
                        ...
                        if (file) {
                          try {
                            const base64 = await compressImage(file);
                            triggerCropper(base64, 'free', 'Potong Scan KTP', (cropped) => {
                              setEditingWarga({
                                ...editingWarga,
                                ktpBase64: cropped,
                                ktpNamaFile: file.name
                    ...
                    <input 
                      ...
                      onChange={async (e) => {
                        ...
                        if (file) {
                          try {
                            const base64 = await compressImage(file);
                            triggerCropper(base64, 'free', 'Potong Scan KK', (cropped) => {
                              setEditingWarga({
                                ...editingWarga,
                                kkBase64: cropped,
                                kkNamaFile: file.name
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-[100] animate-in fade-in duration-200 overflow-y-auto">
          <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-2xl relative animate-in zoom-in-95 duration-200 text-slate-800 max-w-lg w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              {/* Opsi Diskon / Pembebasan Iuran RT */}
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <label htmlFor="enable-pay-diskon" className="text-xs font-extrabold text-purple-900 cursor-pointer flex items-center gap-1.5 select-none">
                      ...
                      Diskon / Pembebasan Iuran RT (Sebab Khusus)
                    </label>
                  </div>
                  {payHasDiskon && (
                    <span className="text-[10px] font-bold text-purple-700 bg-purple-100 px-2 py-0.5 rounded-full">
                      {payDiskonType === 'pembebasan' ? 'BEBAS 100%' : `Diskon Rp ${payDiskonNominal.toLocaleString('id-ID')}`}
                    </span>
                  )}
                </div>

                {payHasDiskon && (
                  <div className="space-y-3 pt-2 border-t border-purple-200/60 animate-in fade-in duration-150">
                    <div className="grid grid-cols-2 gap-2">
                      <button
                        type="button"
                        onClick={() => setPayDiskonType('nominal')}
                      ...
                      <button
                        ...
                        onClick={() => {
                          setPayDiskonType('pembebasan');
                          setPayDiskonNominal(payingInfo.nominal);
                        }}
                        className={`py-1.5 px-3 rounded-xl text-xs font-bold transition flex items-center justify-center gap-1.5 ${
                    ...
                    {payDiskonType === 'nominal' && (
                      <div>
                        <label className="block text-[11px] font-semibold text-purple-900 mb-1">Nominal Diskon / Potongan (Rp)</label>
                        <input
                          type="number"
                          step="1000"
                          min="0"
                          max={payingInfo.nominal}
                    ...
                    {payDiskonAlasan === 'Lainnya (Tulis Sendiri)' && (
                      <div>
                        ...
                        <input
                          ...
                          placeholder="Contoh: Bencana alam, rekomendasi ketua RT, dll..."
                    ...
                    <div className="bg-purple-100/70 border border-purple-200 rounded-xl p-2.5 text-xs text-purple-900 font-mono space-y-1">
                      <div className="flex justify-between">
                        <span>Tarif Normal:</span>
                        <span>Rp {payingInfo.nominal.toLocaleString('id-ID')}</span>
                      </div>
                      <div className="flex justify-between text-rose-600 font-bold">
                        <span>Diskon / Bebas:</span>
                        <span>- Rp {(payDiskonType === 'pembebasan' ? payingInfo.nominal : Math.min(payingInfo.nominal, payDiskonNominal)).toLocaleString('id-ID')}</span>
                      </div>
                      <div className="flex justify-between font-extrabold text-emerald-700 text-sm border-t border-purple-200 pt-1">
                        <span>Total Net Bayar:</span>
                        <span>Rp {(payDiskonType === 'pembebasan' ? 0 : Math.max(0, payingInfo.nominal - payDiskonNominal)).toLocaleString('id-ID')}</span>
                      </div>
                    </div>
                  </div>
                )}
            ...
            <div className="space-y-2 mt-4">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  let diskonText = '';
                  if (receiptSuccessInfo.isPembebasan) {
                    diskonText = `\n• Status Pembebasan: Pembebasan Total 100% (${receiptSuccessInfo.alasanDiskon || 'Kebijakan RT'})`;
                  } else if (receiptSuccessInfo.diskon && receiptSuccessInfo.diskon > 0) {
              ...
              >
                ...
                <span>{receiptSuccessInfo.noWa ? 'Kirim Bukti ke WA Warga' : 'Kirim Bukti via WhatsApp'}</span>
              </button>

              {/* Direct WhatsApp Share without pre-filled number */}
              <button
                type="button"
                onClick={() => {
                  const detailLoc = receiptSuccessInfo.tipe === 'warga'
                ...
                className="w-full bg-teal-600 hover:bg-teal-700 text-white font-bold py-2 rounded-xl cursor-pointer transition text-xs flex items-center justify-center gap-2 active:scale-97 shadow-sm"
              >
                <Send className="w-3.5 h-3.5" />
                <span>Buka WA (Pilih Kontak Langsung 📲)</span>
              </button>
                  ...
                  onClick={async () => {
                    try {
                      ...
                      if (!ClipboardItemClass || !navigator.clipboard || !navigator.clipboard.write) {
                        alert("Salin gambar tidak didukung oleh browser Anda di lingkungan ini. Silaka gunakan tombol Unduh Gambar.");
                        return;
                      }
                      
                      const arr = receiptSuccessPNGUrl.split(',');
              ...
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-150 space-y-3">
                ...
                {corrStatusLunas && (
                  <>
                    ...
                    {/* Diskon / Pembebasan Iuran RT dalam Correction Modal */}
                    <div className="bg-purple-50/70 border border-purple-200 p-3.5 rounded-2xl space-y-2.5">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <input
                            type="checkbox"
                            id="enable-corr-diskon"
                            checked={corrHasDiskon}
                            onChange={(e) => setCorrHasDiskon(e.target.checked)}
                            className="w-4 h-4 text-purple-600 rounded focus:ring-purple-500 cursor-pointer"
                          />
                          <label htmlFor="enable-corr-diskon" className="text-xs font-extrabold text-purple-900 cursor-pointer flex items-center gap-1.5 select-none">
                            <Tag className="w-3.5 h-3.5 text-purple-600" />
                            Koreksi Diskon / Pembebasan Iuran RT
                          </label>
                        </div>
                      </div>
                        ...
                        <div className="space-y-2.5 pt-2 border-t border-purple-200/80">
                          <div className="grid grid-cols-2 gap-2">
                            ...
                            <button
                              type="button"
                              onClick={() => {
                                setCorrDiskonType('pembebasan');
                                setCorrDiskonNominal(corrNominal);
                              }}
                              className={`py-1 px-2.5 rounded-lg text-xs font-bold transition ${
                                corrDiskonType === 'pembebasan'
                          ...
                          {corrDiskonType === 'nominal' && (
                            <div>
                              <label className="block text-[11px] font-semibold text-purple-900 mb-1">Nominal Diskon (Rp)</label>
                              <input
                                type="number"
                                value={corrDiskonNominal}
                                onChange={(e) => setCorrDiskonNominal(Number(e.target.value))}
                                className="w-full bg-white border border-purple-300 rounded-xl p-2 text-xs font-mono font-bold text-purple-950"
                              />
                            </div>
                          )}

                          <div>
                            <label className="block text-[11px] font-semibold text-purple-900 mb-1">Alasan Diskon / Pembebasan</label>
                            <select
                              value={corrDiskonAlasan}
                              onChange={(e) => setCorrDiskonAlasan(e.target.value)}
                              className="w-full bg-white border border-purple-300 rounded-xl p-2 text-xs text-purple-950 font-bold"
                            >
                              {REASONS_DISKON_PEMBEBASAN.map((r, idx) => (
                                <option key={idx} value={r}>{r}</option>
                              ))}
                          ...
                          {corrDiskonAlasan === 'Lainnya (Tulis Sendiri)' && (
                            <div>
                              <input
                                type="text"
                                value={corrDiskonAlasanCustom}
                                onChange={(e) => setCorrDiskonAlasanCustom(e.target.value)}
                                placeholder="Alasan khusus..."
                                className="w-full bg-white border border-purple-300 rounded-xl p-2 text-xs text-purple-950"
                              />
                            </div>
                          )}
                        </div>
                ...
                <div className="p-4 rounded-2xl bg-emerald-50/50 border border-emerald-100">
                  ...
                  <div className="text-lg font-black font-mono text-emerald-800 mt-1">
                    Rp {selectedWargaHistory.iuranRT
                      .filter(b => b.lunas && isMatchSlot(b, b.bulan, historyYear))
                      ...
                      Rp {(() => {
                        ...
                        fullMonths.forEach(m => {
                          const slot = selectedWargaHistory.iuranRT.find(b => 
                            b.lunas && isMatchSlot(b, m, historyYear)
                          );
                  ...
                  {!selectedWargaHistory.anggotaKeluarga || selectedWargaHistory.anggotaKeluarga.length === 0 ? (
                    ...
                    <div className="space-y-1.5 max-h-[220px] overflow-y-auto pr-0.5">
                      {selectedWargaHistory.anggotaKeluarga.map((member, i) => (
                        <div key={member.id || i} className="bg-white border border-slate-150/75 rounded-xl p-2.5 flex items-center gap-3 text-xs">
                          {member.fotoBase64 ? (
                            <img src={member.fotoBase64} alt={member.nama} className="w-9 h-9 rounded-full object-cover border border-slate-200 shrink-0" referrerPolicy="no-referrer" />
                          ) : (
                            <div className="w-9 h-9 rounded-full bg-slate-50 border border-slate-200 flex items-center justify-center shrink-0 text-slate-400">
                              <User className="w-4 h-4" />
                            </div>
                          )}
                          <div className="min-w-0">
                            <div className="font-extrabold text-slate-800 truncate">{member.nama}</div>
                  ...
                  {fullMonths
                    ...
                    .map((IndoMonth) => {
                      ...
                      const matchedSlot = selectedWargaHistory.iuranRT.find(
                        slot => isMatchSlot(slot, IndoMonth, historyYear)
                      );
                          ...
                          {(() => {
                            ...
                            return defaultMonths.map((m) => {
                              const slot = w.iuranRT.find(b => 
                                isMatchSlot(b, m, selectedBillingYear)
                              ...
                              return (
                                <button
                                  ...
                                  onClick={() => {
                                    if (slot.lunas) {
                                      openReceiptReprint(w, 'warga', slot, selectedBillingYear);
                                    } else if (isWargaOfficer) {
                                      openPaymentModal(w, 'Iuran RT', slot.bulan, slot.nominal, 'iuranRT', selectedBillingYear);
                                    }
                                  }}
                                  disabled={!slot.lunas && !isWargaOfficer}
                                  title={slot.lunas ? "Klik untuk Cetak Kuitansi / Kirim WA" : (isWargaOfficer ? "Klik untuk Bayar" : "")}
                                  className={`px-1.5 py-0.5 rounded-lg text-[10px] md:text-[11px] font-bold font-mono text-center transition flex flex-col items-center justify-center min-w-[62px] ${
                                    slot.lunas
                                      ? 'bg-emerald-600 text-white border border-emerald-700 shadow-sm cursor-pointer hover:bg-emerald-700'
                                      : isWargaOfficer
                            ...
                            return defaultMonths.map((m) => {
                              ...
                              return (
                                <button
                                  key={m}
                                  onClick={() => {
                                    if (slot.lunas && !isPendingApp) {
                                      openReceiptReprint(r, 'rombong', slot, selectedBillingYear);
                                    } else if (!slot.lunas && isOfficer) {
                                      openRombongPaymentModal(r, 'Iuran Rombong', slot.bulan, slot.nominal, 'iuranRombong', selectedBillingYear);
                                    }
                                  }}
                                  disabled={(!slot.lunas && !isOfficer) || isPendingApp}
                                  title={isPendingApp ? "Menunggu Persetujuan Admin" : slot.lunas ? "Klik untuk Cetak Kuitansi / Kirim WA" : (isOfficer ? "Klik untuk Bayar" : "")}
                                  className={`px-1.5 py-0.5 rounded-lg text-[10px] md:text-[11px] font-bold font-mono text-center transition flex flex-col items-center justify-center min-w-[62px] ${
                                    isPendingApp
          ...
          <style>{`
            @media print {
              ...
              #printable-billing-area {
                ...
                left: 0;
                top: 0;
                width: ${printDensity === 'normal' ? '100%' : '1200px'} !important;
                max-width: ${printDensity === 'normal' ? '100%' : '1200px'} !important;
                ${printDensity !== 'normal' ? `
                  zoom: ${printDensity === 'super-compact' ? '0.80' : '0.90'} !important;
                  transform: scale(${printDensity === 'super-compact' ? '0.80' : '0.90'}) !important;
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-6 md:p-8 shadow-2xl relative max-w-6xl w-full my-8 flex flex-col max-h-[90vh]">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-slate-100 mb-6 no-print">
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                {printFormatType === 'table' && (
                  <div className="flex items-center bg-slate-100 p-1 rounded-xl border border-slate-200 text-xs no-print gap-1">
                    <span className="text-[10px] font-extrabold text-slate-500 px-1.5 uppercase tracking-wide">Kerapatan:</span>
                    <button
                      onClick={() => setPrintDensity('super-compact')}
                      className={`px-2.5 py-1 rounded-lg font-bold transition cursor-pointer text-[11px] ${
                        printDensity === 'super-compact'
                          ? 'bg-sky-600 text-white shadow-xs'
                          : 'text-slate-600 hover:text-slate-900'
                      }`}
                      title="Sangat Padat (Rekomendasi Pas 1 Halaman A4)"
                    >
                      Pas 1 Hlm A4 (Sangat Padat)
                    </button>
                    <button
                  ...
                  {printFormatType === 'table' && (
                    <button
                      onClick={() => {
                        ...
                        if (printableArea) {
                          const marginSize = printDensity === 'super-compact' ? '4mm 6mm' : printDensity === 'compact' ? '8mm 10mm' : '10mm 12mm';
                          const bodyPadding = printDensity === 'super-compact' ? '2px' : '10px';
                          const marginTop = printDensity === 'super-compact' ? '4px' : '15px';
                          const marginBottom = printDensity === 'super-compact' ? '6px' : '20px';
                          const thtdPadding = printDensity === 'super-compact' ? '3px 4px !important' : printDensity === 'compact' ? '4px 6px !important' : '6px 8px !important';
                          const thtdFontSize = printDensity === 'super-compact' ? '8.5px !important' : printDensity === 'compact' ? '10.5px !important' : '11px !important';
                          const textXsSize = printDensity === 'super-compact' ? '8px !important' : '0.75rem !important';
                          const textSmSize = printDensity === 'super-compact' ? '9.5px !important' : '0.875rem !important';
                          const textBaseSize = printDensity === 'super-compact' ? '11px !important' : '1rem !important';
                          const textLgSize = printDensity === 'super-compact' ? '12px !important' : '1.125rem !important';
                          const textXlSize = printDensity === 'super-compact' ? '13.5px !important' : '1.25rem !important';
                          const mb4Size = printDensity === 'super-compact' ? '0.2rem !important' : '1rem !important';
                          const mb6Size = printDensity === 'super-compact' ? '0.35rem !important' : '1.5rem !important';
                          const mt4Size = printDensity === 'super-compact' ? '0.2rem !important' : '1rem !important';
                          const mt6Size = printDensity === 'super-compact' ? '0.35rem !important' : '1.5rem !important';
                          const mt12Size = printDensity === 'super-compact' ? '0.4rem !important' : '3rem !important';
                          const mb16Size = printDensity === 'super-compact' ? '1.2rem !important' : '4rem !important';
                          const p4Size = printDensity === 'super-compact' ? '0.25rem !important' : '1rem !important';
                          const pb4Size = printDensity === 'super-compact' ? '0.2rem !important' : '1rem !important';
                              ...
                              <html>
                                <head>
                                  ...
                                  <style>
                                    ...
                                    .text-xs { font-size: ${textXsSize}; }
                                    .text-sm { font-size: ${textSmSize}; }
                                    .text-base { font-size: ${textBaseSize}; }
                                    .text-lg { font-size: ${textLgSize}; }
                                    .text-xl { font-size: ${textXlSize}; }
                                    .uppercase { text-transform: uppercase !important; }
                                    .mb-4 { margin-bottom: ${mb4Size}; }
                                    .mb-6 { margin-bottom: ${mb6Size}; }
                                    .mt-4 { margin-top: ${mt4Size}; }
                                    .border-b-4 { border-bottom: 4px solid #000 !important; }
                                    .border-double { border-style: double !important; }
                                    .pb-4 { padding-bottom: ${pb4Size}; }
                                    ...
                                    .mt-6 { margin-top: ${mt6Size}; }
                                    .mt-12 { margin-top: ${mt12Size}; }
                                    .mb-16 { margin-bottom: ${mb16Size}; }
                                    .p-4 { padding: ${p4Size}; }
            ...
            <div className="flex-1 overflow-y-auto pr-1">
              {printFormatType === 'table' ? (
                (() => {
                  const cellPaddingClass = printDensity === 'super-compact' ? 'p-1 text-[9px]' : printDensity === 'compact' ? 'p-1.5 text-[10.5px]' : 'p-2 text-xs';
                  const monthCellPaddingClass = printDensity === 'super-compact' ? 'p-0.5 text-[8px]' : printDensity === 'compact' ? 'p-1 text-[9.5px]' : 'p-1 text-[10px]';
                  const labelClass = printDensity === 'super-compact' ? 'text-[8.5px]' : 'text-[10px]';
                  const valueClass = printDensity === 'super-compact' ? 'text-xs' : 'text-sm';
                  const statGridClass = printDensity === 'super-compact' ? 'gap-2 mt-2 p-2' : printDensity === 'compact' ? 'gap-3 mt-4 p-3' : 'gap-4 mt-6 p-4';
                  const signatureClass = printDensity === 'super-compact' ? 'gap-4 mt-4' : printDensity === 'compact' ? 'gap-6 mt-8' : 'gap-8 mt-12';
                  const signatureMbClass = printDensity === 'super-compact' ? 'mb-8' : printDensity === 'compact' ? 'mb-12' : 'mb-16';

                  return (
                    <div id="printable-billing-area" className={`bg-white border border-slate-150 rounded-2xl shadow-xs text-slate-900 font-sans ${
                      printDensity === 'super-compact' ? 'p-2 md:p-3' : printDensity === 'compact' ? 'p-3 md:p-5' : 'p-4 md:p-8'
                    }`}>
                      
                      {/* Letterhead Header */}
                      <div className={`text-center border-b-4 border-double border-slate-900 ${
                        printDensity === 'super-compact' ? 'pb-1.5 mb-2' : printDensity === 'compact' ? 'pb-2.5 mb-4' : 'pb-4 mb-6'
                      }`}>
                        <h2 className={`font-black font-sans tracking-wide text-slate-900 uppercase leading-tight ${
                          printDensity === 'super-compact' ? 'text-sm md:text-base' : printDensity === 'compact' ? 'text-base md:text-lg' : 'text-lg md:text-xl'
                        }`}>{rtTitle}</h2>
                        <h3 className={`font-extrabold font-sans text-slate-800 tracking-wide uppercase leading-tight mt-1 ${
                          printDensity === 'super-compact' ? 'text-[9.5px]' : printDensity === 'compact' ? 'text-[11px]' : 'text-xs md:text-sm'
                        }`}>{rtAddress}</h3>
                        {rtEmail && (
                          <p className={`text-slate-500 font-medium tracking-wide mt-1 font-sans ${
                            printDensity === 'super-compact' ? 'text-[8.5px]' : 'text-[10px]'
                          }`}>
                            Email: {rtEmail}
                          </p>
                        )}
                      ...
                      <div className={`text-center ${
                        printDensity === 'super-compact' ? 'mb-2' : printDensity === 'compact' ? 'mb-4' : 'mb-6'
                      }`}>
                        <h4 className={`font-extrabold text-slate-900 uppercase tracking-wide ${
                          printDensity === 'super-compact' ? 'text-xs' : printDensity === 'compact' ? 'text-sm' : 'text-base'
                        }`}>
                          {activeSubTab === 'warga' 
                        ...
                        <div className={`font-semibold text-slate-700 mt-0.5 uppercase ${
                          printDensity === 'super-compact' ? 'text-[10px]' : printDensity === 'compact' ? 'text-xs' : 'text-sm'
                        }`}>
                          Tahun Anggaran: <span className="font-mono font-black">{selectedBillingYear}</span>
                        </div>
                        <div className={`text-slate-500 font-mono mt-1 ${
                          printDensity === 'super-compact' ? 'text-[8px]' : 'text-[10px]'
                        }`}>
                        ...
                        <table className="w-full min-w-[1050px] text-left border-collapse border border-slate-400 table-auto text-xs">
                          <thead>
                            <tr className="bg-slate-100 text-slate-900 font-bold border-b border-slate-400">
                              <th className={`border border-slate-400 text-center w-8 ${cellPaddingClass}`}>No</th>
                              {activeSubTab === 'warga' ? (
                                <>
                                  <th className={`border border-slate-400 ${cellPaddingClass}`}>Nama Warga</th>
                                  <th className={`border border-slate-400 text-center w-24 ${cellPaddingClass}`}>Blok &amp; Rumah</th>
                                </>
                              ) : (
                                <>
                                  <th className={`border border-slate-400 ${cellPaddingClass}`}>Pemilik Lapak</th>
                                  <th className={`border border-slate-400 text-center w-24 font-mono ${cellPaddingClass}`}>No Lapak</th>
                                </>
                              )}
                              {fullMonths.map((m) => (
                                <th key={m} className={`border border-slate-400 text-center font-mono ${monthCellPaddingClass}`}>
                                  {m.slice(0, 3)}
                                </th>
                              ))}
                              <th className={`border border-slate-400 text-right w-24 ${cellPaddingClass}`}>Terbayar (Rp)</th>
                              <th className={`border border-slate-400 text-right w-24 ${cellPaddingClass}`}>Tunggak (Rp)</th>
                            </tr>
                          </thead>
                          <tbody>
                            {activeSubTab === 'warga' ? (
                              reportWarga.map((w, idx) => {
                                ...
                                return (
                                  <tr key={w.id} className="hover:bg-slate-50 transition border-b border-slate-350">
                                    <td className={`border border-slate-350 text-center ${cellPaddingClass}`}>{idx + 1}</td>
                                    <td className={`border border-slate-350 font-bold text-slate-900 ${cellPaddingClass}`}>
                                      {w.nama}
                                      {w.isDeleted && (
                                    ...
                                    <td className={`border border-slate-350 text-center font-mono text-slate-800 ${cellPaddingClass}`}>{w.blok}-{w.noRumah}</td>
                                    {monthlyStatus.map((isLunas, mIdx) => (
                                      <td 
                                        key={mIdx} 
                                        className={`border border-slate-350 text-center font-mono font-extrabold ${monthCellPaddingClass} ${
                                    ...
                                    <td className={`border border-slate-350 text-right font-mono text-emerald-800 font-bold ${cellPaddingClass}`}>{paidSum.toLocaleString('id-ID')}</td>
                                    <td className={`border border-slate-350 text-right font-mono font-bold ${unpaidSum > 0 ? 'text-rose-700' : 'text-slate-400'} ${cellPaddingClass}`}>
                              ...
                              reportRombong.map((r, idx) => {
                                ...
                                return (
                                  <tr key={r.id} className="hover:bg-slate-50 transition border-b border-slate-350">
                                    ...
                                    <td className={`border border-slate-350 text-center font-mono text-slate-800 ${cellPaddingClass}`}>{r.noLapak}</td>
                                    {monthlyStatus.map((isLunas, mIdx) => (
                                      <td 
                                        key={mIdx} 
                                        className={`border border-slate-350 text-center font-mono font-extrabold ${monthCellPaddingClass} ${
                      ...
                      <div className={`grid grid-cols-2 md:grid-cols-4 rounded-xl border border-slate-300 bg-slate-50 ${statGridClass}`}>
                        <div>
                          <span className={`uppercase font-bold text-slate-505 block ${labelClass}`}>Total Entitas</span>
                          <strong className={`font-extrabold text-slate-900 ${valueClass}`}>
                            {activeSubTab === 'warga' ? `${reportWarga.length} Kepala Keluarga` : `${reportRombong.length} Lapak Rombong`}
                          </strong>
                        </div>
                        <div>
                          <span className={`uppercase font-bold text-slate-505 block ${labelClass}`}>Estimasi Kas Terbayar</span>
                          <strong className={`font-extrabold text-emerald-800 font-mono ${valueClass}`}>
                        ...
                        <div className="col-span-2 text-right hidden md:block">
                          <span className={`italic text-slate-500 block ${labelClass}`}>* Seluruh pembayaran terhitung dalam sistem akuntansi ganda RT 08</span>
                        </div>
                      </div>

                      {/* Signing Footer Block */}
                      <div className={`grid grid-cols-2 text-center text-xs ${signatureClass}`}>
                        <div>
                          <p className={`font-semibold text-slate-600 ${signatureMbClass}`}>Mengetahui,<br/>Ketua RT 008 RW 004</p>
                        ...
                        <div>
                          <p className={`font-semibold text-slate-605 ${signatureMbClass}`}>Penanggung Jawab Keuangan,<br/>Bendahara RT 008</p>
