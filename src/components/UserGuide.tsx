...
export default function UserGuide({
  ...
  useEffect(() => {
    ...
    setCustomUrl(cfg.supabaseUrl);
    setCustomAnonKey(cfg.supabaseAnonKey);
  }, []);

  const handleSaveKeys = () => {
    if (!customUrl.trim() || !customAnonKey.trim()) {
      setSaveMessage({ type: 'error', text: 'Silakan masukkan URL dan Anon Key yang valid.' });
      return;
    }
    saveCustomSupabaseKeys(customUrl.trim(), customAnonKey.trim());
    setSaveMessage({ type: 'success', text: 'Kredensial Supabase berhasil disimpan! Memuat ulang halaman...' });
    setTimeout(() => {
  ...
  const handleClearKeys = () => {
    ...
    setCustomUrl('');
    setCustomAnonKey('');
    setSaveMessage({ type: 'success', text: 'Kredensial kustom dihapus! Menggunakan konfigurasi default. Memuat ulang halaman...' });
    setTimeout(() => {
      window.location.reload();
    }, 1500);
  };
                ...
                <div className="flex gap-2.5">
                  ...
                  <div className="space-y-1.5 w-full">
                    ...
                    <p className="text-[11px] text-slate-500 leading-normal font-sans">
                      Salin nilai <strong className="text-slate-805">Project URL</strong> dan <strong className="text-slate-805">API Key Anon</strong> dari tab <strong className="text-slate-800 font-semibold">Project Settings &gt; API</strong> di dashboard Supabase Anda. Rekatkan nilai-nilai tersebut di bawah ini untuk menyimpan kredensial ke penyimpanan lokal browser Anda secara aman:
                    </p>

                    <div className="space-y-3 bg-slate-50 border border-slate-200 rounded-xl p-4 mt-2">
                      <div className="space-y-1">
                        <label className="block text-[10.5px] font-semibold text-slate-600">URL Server Supabase</label>
                        <input
                          type="text"
                          value={customUrl}
                          onChange={(e) => setCustomUrl(e.target.value)}
                          placeholder="https://your-project-ref.supabase.co"
                          className="w-full px-3 py-1.5 text-xs border border-slate-250 rounded-lg bg-white focus:outline-none focus:ring-1 focus:ring-sky-500 font-mono text-slate-800"
                        />
                      </div>
                      <div className="space-y-1">
                        <label className="block text-[10.5px] font-semibold text-slate-600">Anon Key / Public API Key</label>
                        <textarea
                          rows={2}
                          value={customAnonKey}
                      ...
                      <div className="flex gap-2 pt-1">
                        <button
                          ...
                          className="flex-1 text-[11px] font-bold text-white bg-sky-600 hover:bg-sky-700 px-3.5 py-1.5 rounded-lg shadow-sm transition active:scale-95 cursor-pointer flex items-center justify-center gap-1"
                        >
                        ...
                        {getSupabaseConfig().isCustom && (
                          <button
                            ...
                            className="text-[11px] font-bold text-slate-700 bg-slate-200 hover:bg-slate-300 px-3 py-1.5 rounded-lg transition active:scale-95 cursor-pointer flex items-center justify-center gap-1"
                          >
                            <span>Reset</span>
                          </button>
                        )}
