...
const safeFetchSaveLocalSync = async (baseUrl: string, payload: any): Promise<any> => {
  ...
  try {
    ...
    // If response body is non-JSON HTML (e.g. from reverse proxy or 413 Body Too Large)
    if (response.status === 413) {
      throw new Error('413: Ukuran data terlalu besar untuk dikirim sekaligus melalui jaringan ini');
    }
    throw new Error(`Server lokal mengembalikan respon non-JSON (Status ${response.status}). Kemungkinan limit transmisi nirkabel/proxy terlampaui.`);
  }

  if (!response.ok) {
    throw new Error(data?.error || `HTTP Error ${response.status}: Silakan periksa koneksi server lokal`);
  }
